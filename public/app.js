  ...
  container.append(
    element(
      ...
      result.services.length
        ? `Найдено услуг: ${result.services.length}`
        : 'Ничего не найдено. Попробуйте «уборка» или «кухня».',
    ),
  )
...
async function loadRequests() {
  ...
  try {
    ...
    if (!requests.length) {
      ...
      empty.append(
        element('strong', 'Заявок пока нет'),
        element('span', 'Выберите услугу и удобную дату в форме.'),
      )
...
if (document.getElementById('registered-message')) {
  document.getElementById('registered-message').hidden = !new URLSearchParams(location.search).has(
    'registered',
  )
}

// Состояние сессии определяет меню, доступ к кабинету и показ cookie-уведомления.
const ready = api('/api/session')
  .catch(() => {
    throw new Error(
      'Не удалось загрузить сессию с сервера. Проверьте соединение и обновите страницу.',
    )
  })
  .then(async (state) => {
...
document.getElementById('accept-cookies')?.addEventListener('click', async (event) => {
  ...
  try {
    ...
    document.getElementById('cookie-error').textContent = errorMessage(error)
  } finally {
    button.disabled = false
  }
})
